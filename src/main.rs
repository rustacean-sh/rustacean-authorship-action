...
async fn main() -> Result<()> {
    ...
    let token = args.get(3).context("Github token is required")?;

    if pr_author.is_empty() {
        bail!("PR Author Cannot Be Empty!");
    }

    let rustacean = RustaceanMember::new(
        pr_author.to_string(),
        pr_number.to_string(),
        token.to_string(),
    );
    ...
    println!(
        "Hello {pr_author} your PR number is {pr_number}, and your changes are {}",
